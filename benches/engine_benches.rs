...
use tet_core::memory::{VectorVfs, VectorRecord, SearchQuery};
...
fn bench_wasm_invoke(c: &mut Criterion) {
    ...
    for size in [20, 80, 320].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &scale| {
            b.to_async(&rt).iter(|| run_concurrent_invokes(scale, wasm.clone(), sbox.clone()));
        });
    }
    group.finish();
}
...
async fn run_vector_vfs(scale: usize, vfs: Arc<VectorVfs>) {
    ...
    for i in 0..scale {
        ...
        handles.push(tokio::spawn(async move {
            ...
            vfs_clone.remember("default", VectorRecord {
                id: format!("fact_{}", i),
...
fn bench_teleport_encode(c: &mut Criterion) {
    ...
    group.bench_function("bincode_serialize_large", |b| b.iter(|| {
        bincode::serialize(&sim_payload).unwrap()
    }));
    group.finish();
}
