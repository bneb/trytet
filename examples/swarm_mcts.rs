...
fn load_python_evaluator_wasm() -> Vec<u8> {
    ...
    std::fs::read(&path)
        .unwrap_or_else(|_| panic!("Failed to read Python Evaluator WASM at {:?}.", path))
}
...
fn main() {
    ...
    println!(
        "{}",
        "   Trytet Autonomous Swarm MCTS Demonstration"
            .bold()
            .green()
    );
    println!(
    ...
    mgr.precompile("python", &wasm)
        .expect("Failed to precompile python-evaluator");
    println!(
        "{} Python Cartridge precompiled. Sandboxes are hot.\n",
        "[SYSTEM]".bold().cyan()
    );

    println!(
        "{} Initializing 500 Agent Swarm nodes...",
        "[SWARM]".bold().magenta()
    );
    println!(
        "{} Objective: Find the Python permutation that yields the value '100'.\n",
        "[SWARM]".bold().magenta()
    );
    ...
    for branch_id in 1..=total_branches {
        ...
        match result {
            Ok((output, _metrics)) => {
                if output == "100" {
                    println!(
                        "{} {} 🎯 SUCCESS: Found correct solution! ({}µs)",
                        node_label,
                        "✓".bold().green(),
                        duration.as_micros()
                    );
                    ...
                    println!(
                        "{} {} ❌ Logic evaluated but output '{}' is incorrect. ({}µs)",
                        node_label,
                        "⨯".bold().red(),
                        output,
                        duration.as_micros()
            ...
            Err(CartridgeError::FuelExhausted) => {
                println!(
                    "{} {} ⚠️  TRAPPED: Fuel Exhausted (Infinite Loop) ({}µs)",
                    node_label,
                    "⚡".bold().yellow(),
                    duration.as_micros()
                );
            ...
            Err(CartridgeError::MemoryExceeded) | Err(CartridgeError::ExecutionError(..)) => {
                println!(
                    "{} {} ⚠️  TRAPPED: Memory Exceeded (Memory Bomb) ({}µs)",
                    node_label,
                    "💥".bold().magenta(),
                    duration.as_micros()
                );
    ...
    println!(
        "{} All malicious logic deterministically trapped.",
        "[SYSTEM]".bold().cyan()
    );
    println!(
        "{}\n",
