...
fn main() {
    ...
    println!(
        "{}",
        "   Trytet 'Data Agent' Regex Extraction Demo"
            .bold()
            .green()
    );
    println!(
    ...
    println!(
        "{} LLM Agent generated 100 candidate regex patterns.",
        "[AGENT]".bold().magenta()
    );
    println!(
        "{} Objective: Extract the TRYTET-ID from unstructured log data.",
        "[AGENT]".bold().magenta()
    );
    println!(
        "{} Fanning out verification to Trytet Sandboxes...\n",
        "[SYSTEM]".bold().cyan()
    );
    ...
    for candidate_id in 1..=total_candidates {
        ...
        match result {
            ...
            Err(CartridgeError::MemoryExceeded) | Err(CartridgeError::ExecutionError(..)) => {
                println!(
                    "{} {} ⚠️  TRAPPED: Memory Exceeded - Massive Allocation Detected! ({}µs)",
                    node_label,
                    "💥".bold().magenta(),
                    duration.as_micros()
                );
    ...
    println!(
        "{} ReDoS logic bombs trapped instantly. Zero host degradation.",
        "[SYSTEM]".bold().cyan()
    );
    println!(
        "{}\n",
