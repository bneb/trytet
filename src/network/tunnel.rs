...
pub enum TunnelError {
    ...
    RekeyRequired,
    #[error("Invalid key material: expected at least 32 bytes, got {0}")]
    InvalidKey(usize),
}
...
impl Tunnel {
    pub fn init_initiator_nn() -> Result<Self, TunnelError> {
        let builder = Builder::new(
            "Noise_NN_25519_ChaChaPoly_BLAKE2s"
                .parse()
                .expect("invalid noise protocol"),
        );
        let handshake = builder.build_initiator()?;
        Ok(Self {
            noise_state: Some(handshake),
            transport: None,
            transfer_count: 0,
        })
    }
    ...
    pub fn init_initiator(local_secret: &[u8], remote_pubkey: &[u8]) -> Result<Self, TunnelError> {
        ...
        let mut key = [0u8; 32];
        let bytes = local_secret
            .get(..32)
            .ok_or(TunnelError::InvalidKey(local_secret.len()))?;
    ...
    pub fn init_responder(
        ...
        let handshake = builder.local_private_key(&key).build_responder()?;
    ...
    pub fn encrypt_command(&mut self, cmd: &HiveCommand) -> Result<Vec<u8>, TunnelError> {
        ...
        let mut final_out =
            Vec::with_capacity(payload.len() + (payload.len() / chunk_size + 1) * 20);
        ...
        for chunk in payload.chunks(chunk_size) {
            let len = if let Some(transport) = &mut self.transport {
                ...
                return Err(TunnelError::IoError(std::io::Error::other(
    ...
    pub fn decrypt_payload(&mut self, payload: &[u8]) -> Result<HiveCommand, TunnelError> {
        ...
        while offset < payload.len() {
            if offset + 4 > payload.len() {
                return Err(TunnelError::IoError(std::io::Error::new(
