...
impl StudioOrchestrator {
    pub fn new(toml_str: &str) -> anyhow::Result<Self> {
        ...
        for agent in &manifest.agents {
            if !aliases.insert(agent.alias.clone()) {
                anyhow::bail!(
                    "Invalid Manifest: Duplicate alias detected -> {}",
                    agent.alias
                );
            }
        }
    ...
    pub async fn up(
        &self,
        sandbox: std::sync::Arc<dyn TetSandbox>,
    ) -> anyhow::Result<Vec<(String, ExecutionStatus)>> {
        ...
        for agent in &self.manifest.agents {
            ...
            let wasm_bytes = if std::path::Path::new(&agent.base).exists() {
                ...
                vec![
                    ...
                    0x00, 0x03, 0x02, 0x01, 0x00, 0x05, 0x03, 0x01, 0x00, 0x01, 0x07, 0x07, 0x01,
                    0x03, 0x72, 0x75, 0x6e, 0x00, 0x00, 0x0a, 0x04, 0x01, 0x02, 0x00, 0x0b,
                ] // valid empty module
            };
            ...
            join_set.spawn(async move {
                ...
                let status = res.map(|r| r.status).unwrap_or_else(|e| {
                    ExecutionStatus::Crash(crate::models::CrashReport {
                        error_type: crate::models::CrashType::EngineSpawn,
