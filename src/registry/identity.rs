...
impl IdentityRegistry {
    pub fn new(dht_handle: Arc<dyn GlobalRegistry>) -> Self {
        ...
        if !local_cache.exists() {
            fs::create_dir_all(&local_cache).expect("failed to create cache dir");
        }
    ...
    pub async fn push_artifact(
        ...
        self.dht_handle
            ...
            .await
            .map_err(|_| RegistryError::ResolutionFailed)?;
    ...
    pub async fn pull_artifact(
        &self,
        alias: &str,
        _peers: Option<HivePeers>,
    ) -> Result<(ArtifactManifest, Vec<u8>), RegistryError> {
        let route_opt = self
            .dht_handle
            .resolve_alias(alias)
            .await
        ...
        if let Some(route_data) = route_opt {
            let manifest: ArtifactManifest =
                serde_json::from_str(&route_data).map_err(|_| RegistryError::ResolutionFailed)?;
    ...
    pub async fn pull_artifact_from_mesh(
        &self,
        _alias: &str,
        manifest: &ArtifactManifest,
    ) -> Result<(ArtifactManifest, Vec<u8>), RegistryError> {
