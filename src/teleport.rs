use crate::hive::{link::MigrationPacket, HiveClient, HiveCommand};
...
impl TeleportRequest {
    pub async fn execute(
        ...
        info!(
            "Teleportation sequence initiated for agent: {}",
            self.agent_id
        );
        ...
        if self.use_registry {
            let registry = registry_client.ok_or_else(|| {
                TeleportError::Network(anyhow::anyhow!("Registry client not configured"))
            })?;
            let reference = format!("{}-state:temp", self.agent_id);

            registry
                .push_state(&payload, &reference)
                .await
                .map_err(|e| {
                    TeleportError::Network(anyhow::anyhow!("Registry push failed: {}", e))
                })?;
            ...
            HiveClient::send_command(
                &self.target_address,
                HiveCommand::Migration(crate::hive::HiveMigrationCommand::MigrationPacket(
                    MigrationPacket::Handshake {
                        manifest: manifest.clone(),
                        snapshot_id: snapshot_id.clone(),
                    },
                )),
            ...
            let total_payload = bincode::serialize(&payload).map_err(|e| {
                TeleportError::Network(anyhow!("Failed to serialize payload: {}", e))
            })?;

            let chunk_size = 64 * 1024; // 64KB chunks
            for (chunk_idx, chunk) in total_payload.chunks(chunk_size).enumerate() {
                let sequence = chunk_idx as u32;
                HiveClient::send_command(
                    &self.target_address,
                    HiveCommand::Migration(crate::hive::HiveMigrationCommand::MigrationPacket(
                        MigrationPacket::Payload {
                            chunk: chunk.to_vec(),
                            sequence,
                        },
                    )),
            ...
            HiveClient::send_command(
                &self.target_address,
                HiveCommand::Migration(crate::hive::HiveMigrationCommand::MigrationPacket(
                    MigrationPacket::Commit { signature: vec![] },
                )),
            )
            .await
            .map_err(|e| TeleportError::TargetError(e.to_string()))?;
        ...
        // 4. Update the Swarm DHT Routing implicitly via the Gateway trait so active listener requests follow the soul.
        let wallet =
            crate::crypto::AgentWallet::load_or_create().expect("failed to load agent wallet");
        let _ = sandbox
            .publish_dht_route(
                &self.agent_id,
                &self.target_address,
