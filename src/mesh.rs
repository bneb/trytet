...
//!
//! Mesh discovery and routing between agents using aliases
...
#[derive(Clone)]
pub struct TetMesh {
    /// Registry mapping aliases to active instances
...
impl TetMesh {
    ...
    pub async fn record_telemetry(
        ...
        edge.last_seen_ns = SystemTime::now()
            ...
            .expect("failed to register agent in mesh")
            .as_nanos() as u64;
    }
    ...
    pub async fn resolve(&self, alias: &str) -> Option<TetMetadata> {
        ...
        for target_node in nodes {
            let cmd = crate::hive::HiveCommand::Dht(crate::hive::HiveDhtCommand::ResolveAlias(
                alias.to_string(),
            ));
            if let Ok(crate::hive::HiveCommand::Dht(
                crate::hive::HiveDhtCommand::ResolveAliasResponse(Some(meta)),
