...
use crate::models::{
    MeshCallRequest, MeshCallResponse, SnapshotResponse, TetExecutionRequest, TetExecutionResult,
    TopologyEdge,
};
...
impl TetSandbox for WebNativeSandbox {
    ...
    async fn send_mesh_call(&self, req: MeshCallRequest) -> Result<MeshCallResponse, TetError> {
        Err(TetError::MeshError(
            "Mesh calls not supported on web natively".into(),
        ))
    }
    ...
    async fn query_memory(
        ...
        Err(TetError::EngineError(
            "Memory Query not implemented in browser".into(),
        ))
    }
    ...
    async fn infer(
        ...
        fuel_allowance: u64,
    ) -> Result<InferenceResponse, TetError> {
        Err(TetError::InferenceError(
            "Inference not implemented in browser".into(),
        ))
    }
}
