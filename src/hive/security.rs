use rustls::{ServerConfig, ClientConfig, RootCertStore, pki_types::CertificateDer, pki_types::PrivateKeyDer, server::WebPkiClientVerifier};
use std::sync::Arc;
...
pub fn create_secure_hive_server(
    ...
    let mut config = if let Some(roots) = trusted_roots {
        ...
        builder.with_client_cert_verifier(client_auth).with_single_cert(cert_chain, key_der)?
    } else {
        builder.with_no_client_auth().with_single_cert(cert_chain, key_der)?
    };
