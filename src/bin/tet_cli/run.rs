...
pub async fn build_cmd(entry: &std::path::Path, out: &std::path::Path) -> Result<()> {
    ...
    pb.set_message(format!(
        "Compiling {} to Trytet Agent Component...",
        entry.display()
    ));
    ...
    let local_path = std::env::current_dir()
        ...
        .join("crates")
        .join("js-evaluator")
        .join("target/wasm32-wasip1/release/js_evaluator.wasm");
    let global_path = base_dir.join("js_evaluator.wasm");

    let wasm_path = if local_path.exists() {
        local_path
    } else {
        global_path
    };
    ...
    let wasm_bytes = fs::read(&wasm_path).unwrap_or_else(|_| {
        println!(
            "{} Warning: js_evaluator.wasm not found. Building empty agent shell.",
            "⚠".yellow()
        );
        vec![]
    });
    ...
    pb.finish_with_message(format!(
        "{} Successfully compiled agent artifact to {}",
        "✔".green(),
        out.display()
    ));
    Ok(())
...
pub async fn replay_cmd(client: &Client, snapshot_id: &str, payload: Option<&str>) -> Result<()> {
    if snapshot_id.contains('/') || snapshot_id.contains('\\') || snapshot_id.contains("..") {
        return Err(anyhow!(
            "Invalid snapshot ID: contains path traversal characters"
        ));
    }
...
pub async fn up_artifact(file: &std::path::PathBuf, fuel: Option<u64>) -> anyhow::Result<()> {
    let pb = ProgressBar::new_spinner();
    pb.set_style(pb_style());
    pb.set_message(format!("Resurrecting agent from '{}'...", file.display()));
