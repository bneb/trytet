...
impl LlamaCppEngine {
    ...
    async fn get_raw_model(&self, alias: &str) -> Option<LlamaModel> {
        ...
        let model = LlamaModel::load_from_file(&self.backend, &info.path, &model_params)
            .ok()?;
...
impl NeuralEngine for LlamaCppEngine {
    ...
    async fn predict(&self, request: &InferenceRequest, fuel_limit: u64) -> Result<InferenceResponse, String> {
        ...
        let model = self.get_raw_model(&request.model_alias).await
        ...
        let session = self.registry.get_or_create_session(&session_id, &request.model_alias).await;
        ...
        let (text_generated, tokens_generated, prompt_tokens, is_trap) = tokio::task::spawn_blocking(move || {
            ...
            let mut ctx = model.new_context(&backend_clone, ctx_params)
            ...
            while generated < req_clone.max_tokens {
                ...
                let mut candidates_p = LlamaTokenDataArray::from_iter(candidates.into_iter(), false);
                let new_token_id = candidates_p.sample_token_greedy();

                if new_token_id == model.token_eos() || req_clone.stop_sequences.iter().any(|s| output_text.contains(s)) {
                    break;
                }

                let token_bytes = model.token_to_piece_bytes(new_token_id, 8, false, None)
                ...
                let current_fuel_cost = InferenceFuelCalculator::calculate(prompt_tokens, generated);
                ...
                ctx.decode(&mut batch).map_err(|e| format!("Autoregressive decode failed: {}", e))?;
            }
        ...
        let mut final_session = self.registry.get_or_create_session(&session_id, &request.model_alias).await;
