//! HTTP Oracle
//!
//! Bridges sandboxed agents to external HTTP APIs via
...
pub struct SignedTruth {
    ...
    /// Model identifier for inference-specific cache hits.
    #[serde(default)]
    pub model_id: Option<String>,
    /// Temperature parameter for inference-specific cache hits.
    #[serde(default)]
...
impl MeshOracle {
    ...
    /// Shared request pipeline: hash, check cache, send HTTP, sign, write cache.
    async fn resolve_inner(
        ...
        // Cache hit — return immediately (headers do not affect the cache key)
        ...
        tokio::fs::write(&cache_file, truth_bytes).await?;
    ...
    pub async fn resolve(
        ...
        self.resolve_inner(req, cache_dir, None).await
    }

    /// Resolve with injected identity headers.
    ...
    pub async fn resolve_with_headers(
        ...
        self.resolve_inner(req, cache_dir, Some(&extra_headers))
            .await
    }

    /// Resolve an inference-specific Oracle lookup.
