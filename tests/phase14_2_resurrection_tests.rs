...
use tet_core::resurrection::ResurrectionContext;
...
async fn test_lazarus_resurrection() {
    ...
    assert!(
        result_file.exists(),
        "out.txt did not persist into node_workspace VFS"
    );
...
async fn test_tainted_soul_integrity() {
    ...
    assert!(
        ...
        "Tainted artifact loaded without triggering security trap!"
    );
    ...
    match verify_result {
        Err(e) => {
            ...
            assert!(
                ...
                "Expected Integrity Error trap, got: {}",
                es
            );
        }
