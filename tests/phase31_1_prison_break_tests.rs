...
async fn test_path_traversal_jail() {
    ...
    assert!(
        matches!(
            jailer.safe_join("malicious.txt\0.exe"),
            Err(SecurityError::PathTraversalAttempt)
        ),
...
async fn test_memory_wrap_escape() {
    ...
    match result.status {
        tet_core::models::ExecutionStatus::Crash(rpt) => {
            assert!(
                rpt.message.contains("OOB Guest Memory"),
                "Did not receive proper OOB Trap: {:?}",
                rpt.message
            );
        }
...
async fn test_inference_dos_preemption() {
    ...
    match result.status {
        tet_core::models::ExecutionStatus::Crash(rpt) => {
            assert!(
                rpt.message.contains("Resource Exhaustion Attempt Detected"),
                "Must correctly relay SecurityError: {}",
                rpt.message
            );
        }
