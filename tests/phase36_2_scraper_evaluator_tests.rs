...
fn load_scraper_evaluator_wasm() -> Vec<u8> {
    let candidates = &[
        "crates/scraper-cartridge/target/wasm32-wasip1/release/scraper_cartridge.wasm",
        "dist/cartridges/scraper_cartridge.wasm",
    ];
    ...
    panic!("Scraper .wasm not found. Tried: {:?}", candidates)
}
...
fn test_scraper_evaluator_success() {
    ...
    mgr.precompile("scraper-cartridge", &wasm)
        .expect("Failed to precompile scraper-cartridge");
...
fn test_scraper_evaluator_fuel_exhaustion() {
    ...
    let payload_str = format!(
        r#"{{"html": "<html><body><main>{}</main></body></html>", "selector": "p"}}"#,
        huge_p
    );
