...
fn test_cartridge_micro_latency_spinup() {
    ...
    // p99 is dominated by first-call Cranelift compilation (~400ms release,
    // ~550ms debug). Cached invocations are <500µs. This ceiling is only
    // enforced in --release mode (see BENCHMARKS.md).
    if cfg!(debug_assertions) {
        eprintln!(
            "Cartridge spin-up p99 (debug): {}µs (ceiling enforced in --release only)",
            p99
        );
    } else {
        assert!(
            p99 < 2_000,
            "Cartridge spin-up p99 exceeded ceiling: p99={}µs (ceiling=2000µs)",
            p99
        );
    }
}
...
fn test_cartridge_not_found_returns_registry_error() {
    ...
    match result {
        ...
        other => panic!("Expected CartridgeError::RegistryError, got: {:?}", other),
    }
}
...
fn test_cartridge_network_fetch_capability() {
    ...
    match result {
        Ok((output, metrics)) => {
            ...
            assert!(
                metrics.fuel_consumed >= 5_000,
                "Fuel should be consumed for the fetch operation"
            );
        }
