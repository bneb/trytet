...
async fn test_phase_26_migration_sanity() {
    ...
    let imported_tet_id = node_b
        .import_snapshot(payload_obj)
        .await
        .expect("Failed to import to B");

    assert!(
        !imported_tet_id.is_empty(),
        "Import should return a valid Tet ID representing the active execution context"
    );
}
