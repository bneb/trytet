...
fn load_regex_evaluator_wasm() -> Vec<u8> {
    let candidates = &[
        "crates/regex-evaluator/target/wasm32-wasip1/release/regex_evaluator.wasm",
        "dist/cartridges/regex_evaluator.wasm",
    ];
    ...
    panic!("Regex evaluator .wasm not found. Tried: {:?}", candidates)
}
...
fn test_regex_evaluator_success() {
    ...
    mgr.precompile("regex-evaluator", &wasm)
        .expect("Failed to precompile regex-evaluator");
    ...
    let (output, metrics) =
        result.expect("Regex Evaluator should successfully match simple pattern");

    assert!(
        output.contains(r#""matched":true"#),
        "Output was: {}",
        output
    );
...
fn test_regex_evaluator_redos_protection() {
    ...
    match result {
        ...
        other => panic!("Expected safe return or FuelExhausted, got: {:?}", other),
    }
}
