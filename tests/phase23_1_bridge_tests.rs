...
async fn test_phase23_atomic_burn_with_withdraw() {
    ...
    let cmd = HiveCommand::Economy(tet_core::hive::HiveEconomyCommand::WithdrawalPending(
        intent,
    ));

    assert!(
        matches!(cmd, HiveCommand::Economy(tet_core::hive::HiveEconomyCommand::WithdrawalPending(ref i)) if i.internal_fuel == 500_000)
    );
}
