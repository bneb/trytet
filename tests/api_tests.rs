...
/// Create a fresh temp directory for each test's local registry.
fn test_registry_dir() -> std::path::PathBuf {
    let p = std::env::temp_dir().join(format!("tet-api-test-reg-{}", std::process::id()));
...
fn test_app() -> axum::Router {
    ...
    let state = Arc::new(AppState {
        sandbox,
        registry: Arc::new(
            tet_core::registry::LocalRegistry::new(config.registry_path.clone()).unwrap(),
        ),
...
fn test_app_identity(node_id: &str) -> axum::Router {
    let config = test_config();
    let hive_peers = tet_core::hive::HivePeers::new();
    let (mesh, call_rx) = tet_core::mesh::TetMesh::new(10, hive_peers.clone());
    let sandbox = Arc::new(
...
async fn test_health_check() {
    ...
    // The handler returns 503 when /data/registry is missing (typical in test env),
    // or 200 when the path exists. Accept either.
    assert!(
    ...
    assert!(json["status"].is_string(), "Expected 'status' string field");
    assert!(json["region"].is_string(), "Expected 'region' string field");
}
...
async fn test_phase_11_economic_rejection() {
    let app = test_app_identity("node_alpha");
    ...
    match result.status {
        tet_core::models::ExecutionStatus::Crash(report) => {
            assert_eq!(
                report.error_type,
                tet_core::models::CrashType::EconomicViolation
            );
...
async fn test_phase_12_ransom_test() {
    ...
    let app = test_app_identity(&node_id);
...
async fn test_phase_14_secure_egress() {
    ...
    match &result.status {
        tet_core::models::ExecutionStatus::Crash(cr) => {
            assert_eq!(
                cr.error_type,
                tet_core::models::CrashType::SecurityViolation
            );
        }
        _ => panic!(
...
#[tokio::test]
#[ignore = "Ingress registration stores in AppState.ingress_routes but proxy reads gateway.local_routes -- needs code fix to bridge the two"]
...
#[tokio::test]
#[ignore = "VectorVfs memory_shards not initialized through sandbox execution path -- panics in get_shard"]
