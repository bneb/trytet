...
fn test_registry_dir() -> std::path::PathBuf {
    let p = std::env::temp_dir().join(format!("tet-cli-test-reg-{}", std::process::id()));
...
#[tokio::test]
#[ignore = "Requires running tet CLI binary and mock API server -- run manually with cargo test -- --ignored"]
...
#[tokio::test]
#[ignore = "Requires multi-node Hive setup with running tet CLI binary -- run manually with cargo test -- --ignored"]
async fn test_phase_10_live_migration() {
    ...
    let join = tet_core::hive::HiveCommand::Network(tet_core::hive::HiveNetworkCommand::Join(
        tet_core::hive::HiveNodeIdentity {
