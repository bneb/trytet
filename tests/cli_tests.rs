...
async fn spawn_mock_api_server() -> (u16, u16) {
    ...
    let app_state = Arc::new(AppState { sandbox: sandbox.clone(), registry, hive: Some(hive_peers.clone()), ingress_routes: Arc::new(tokio::sync::RwLock::new(std::collections::HashMap::new())) });
...
async fn test_phase_8_git_for_ram_registry() {
    ...
    if !output.status.success() {
        panic!("tet run failed. stderr: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    if !snap_out.status.success() {
        panic!("tet snapshot failed. stderr: {}", String::from_utf8_lossy(&snap_out.stderr));
    }
    ...
    if !push_out.status.success() {
        panic!("tet push failed. stderr: {}", String::from_utf8_lossy(&push_out.stderr));
    }
    ...
    if !pull_out.status.success() {
        panic!("tet pull failed. stderr: {}", String::from_utf8_lossy(&pull_out.stderr));
    }
}
...
async fn test_phase_9_base_tet_injection() {
    ...
    if !output.status.success() {
        panic!("tet run py failed. stderr: {}", String::from_utf8_lossy(&output.stderr));
    }
...
async fn test_phase_10_live_migration() {
    ...
    tet_core::hive::HiveClient::rpc_call(
        &format!("127.0.0.1:{}", hive_port1), 
        join
    ).await.unwrap();
    ...
    assert!(teleport_out.status.success(), "Teleport command failed: STDOUT: {}\nSTDERR: {}", String::from_utf8_lossy(&teleport_out.stdout), String::from_utf8_lossy(&teleport_out.stderr));
    ...
    if !snap_out.status.success() {
        panic!("Migration verification failed! Node 2 could not snapshot the alias. stderr: {}", String::from_utf8_lossy(&snap_out.stderr));
    }
}
