use std::sync::Arc;
use tet_core::network::vitality::{Heartbeat, NodeStatus, VitalityManager};
use tet_core::registry::identity::IdentityRegistry;
use tet_core::runtime::recovery::{RecoveryError, RecoveryOrchestrator};
...
async fn test_recovery_fidelity() {
    ...
    let sandbox =
        Arc::new(WasmtimeSandbox::new(mesh, voucher_manager, false, "system".into()).unwrap());
    ...
    impl tet_core::gateway::GlobalRegistry for MockGlobalRegistry {
        fn resolve_alias(
            ...
            '_,
            Result<Option<String>, tet_core::gateway::GatewayError>,
        > {
        ...
        fn update_route(
            ...
            _node_ip: &str,
            _signature: &str,
        ) -> futures_util::future::BoxFuture<'_, Result<(), tet_core::gateway::GatewayError>>
        {
            Box::pin(async { Ok(()) })
        }
    }
    ...
    let registry = Arc::new(IdentityRegistry::new(
        Arc::new(MockGlobalRegistry) as Arc<dyn tet_core::gateway::GlobalRegistry>
    ));
    ...
    match result {
        ...
        _ => panic!("Resurrected agent failed to invoke proper Identity Registry recovery stack!"),
    }
}
