...
async fn test_executor_rescue() {
    ...
    let snapshot_id = sandbox.snapshot(&res.tet_id).await.expect("Snapshot failed");
    ...
    assert!(drift < 10, "Executor drift was {}ms, expected < 10ms", drift);
}
...
async fn test_shadow_index_vfs() {
    ...
    for i in 0..2048 {
        vfs.remember(collection, tet_core::memory::VectorRecord {
            id: format!("rec-{}", i),
    ...
    tokio::spawn(async move {
        vfs_compact.remember("test_col", tet_core::memory::VectorRecord {
    ...
    for t in read_tasks {
        ...
        assert!(elapsed.as_millis() < 5, "Latency spike detected, possible lock convoy!");
    }
}
...
async fn test_deterministic_shield() {
    ...
    let res: Result<SnapshotPayload, _> = bincode::options().with_limit(tet_core::sandbox::sandbox_wasmtime::MAX_SNAPSHOT_SIZE).deserialize(&encoded);
    assert!(res.is_err(), "Bincode should reject blobs larger than MAX_SNAPSHOT_SIZE");
}
