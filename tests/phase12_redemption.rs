...
async fn test_executor_rescue() {
    ...
    let ceiling = 100; // Allow generous drift — CI and loaded machines routinely exceed 10ms
    assert!(
        drift < ceiling,
        "Executor drift was {}ms, expected < {}ms",
        drift,
        ceiling
    );
}
...
async fn test_shadow_index_vfs() {
    ...
    for t in read_tasks {
        ...
        assert!(
            ...
            "Latency spike detected, possible lock convoy! ({}ms)",
            elapsed.as_millis(),
        );
    }
}
