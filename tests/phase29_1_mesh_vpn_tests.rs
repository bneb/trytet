...
use tet_core::network::tunnel::Tunnel;
...
async fn test_identity_bound_handshake_rejection() {
    ...
    let tunnel_result = Tunnel::init_initiator(&attacker_kp.private, &[0u8; 32]);
    // It should not panic on build, but it will fail on handshake!
    let mut attacker = tunnel_result.unwrap();

    let mut responder =
        Tunnel::init_responder(&responder_kp.private, &responder_kp.public).unwrap();
...
async fn test_eavesdropper_silence() {
    ...
    let mut initiator = Tunnel::init_initiator(&init_kp.private, &resp_kp.public).unwrap();
...
async fn test_forward_secrecy_and_throughput() {
    ...
    let mut initiator = Tunnel::init_initiator(&init_kp.private, &resp_kp.public).unwrap();
    let mut responder = Tunnel::init_responder(&resp_kp.private, &resp_kp.public).unwrap();
    ...
    println!(
        "Ser: {}ms, Enc: {}ms",
        ser_dur.as_millis(),
        encrypt_dur.as_millis()
    );

    // Debug builds are too variable for hard ceilings; criterion benchmarks
    // in --release mode provide the authoritative measurement (see BENCHMARKS.md).
    if cfg!(debug_assertions) {
        eprintln!(
            "VPN encryption (debug): Ser: {}ms, Enc: {}ms (ceiling enforced in --release only)",
            ser_dur.as_millis(),
            encrypt_dur.as_millis()
        );
    } else {
        assert!(
            ...
            "Encryption latency {}ms exceeds ceiling 500ms (Ser: {}ms)",
            encrypt_dur.as_millis(),
            ser_dur.as_millis()
        );
    }
    ...
    if cfg!(debug_assertions) {
        eprintln!(
            "VPN decryption (debug): {}ms (ceiling enforced in --release only)",
            dec_dur.as_millis()
        );
    } else {
        assert!(
            dec_dur.as_millis() < 500,
            "Decryption latency {}ms exceeds ceiling 500ms",
            dec_dur.as_millis()
        );
    }
}
