...
use tet_core::fortress::{IdentityHeaders, QuotaManager, TenantNamespace};
...
// Construct IdentityHeaders and verify:
...
fn test_authenticated_egress() {
    ...
    let headers = IdentityHeaders::inject(tet_id, author_pubkey, &wallet, method, url, body);

    // Verify exactly 3 headers returned
    assert_eq!(headers.len(), 3, "Expected exactly 3 identity headers");
