...
fn load_js_evaluator_wasm() -> Vec<u8> {
    ...
    panic!(
        "JS evaluator .wasm not found. Tried: {:?}. Build it with: cd crates/js-evaluator && cargo component build --release --target wasm32-wasip1",
        candidates
    )
}
...
fn test_js_evaluator_success() {
    ...
    mgr.precompile("js-evaluator", &wasm)
        .expect("Failed to precompile js-evaluator");
