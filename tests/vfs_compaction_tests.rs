use tet_core::memory::{VectorVfs, VectorRecord};
...
async fn test_vfs_concurrent_compaction() {
    ...
    for t in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            for i in 0..250 { // 250 * 10 = 2500 breaches 2048
                let vec = vec![0.1; 64]; 
    ...
    assert!(max_dur < std::time::Duration::from_millis(500), "remember() stalled caller!");
    drop(vfs);
}
