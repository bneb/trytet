...
use tet_core::hive::security::{create_secure_hive_server, create_secure_hive_client};
use tet_core::sandbox::WasmtimeSandbox;
use tet_core::mesh::TetMesh;
use tet_core::economy::VoucherManager;
use rcgen::generate_simple_self_signed;

fn generate_cert(subject_alt_names: Vec<String>) -> (Vec<CertificateDer<'static>>, PrivateKeyDer<'static>) {
...
async fn test_untrusted_peer_rejected() {
    ...
    let tls_acceptor = create_secure_hive_server(target_cert_chain.clone(), target_key, Some(trusted_roots.clone())).unwrap();
    ...
    let target_sandbox = Arc::new(WasmtimeSandbox::new(mesh.clone(), voucher_manager, false, "target".to_string()).unwrap());
    ...
    tokio::spawn(async move {
        target_server.start(target_port, target_mesh, target_sandbox_clone).await.unwrap();
    });
    ...
    let tls_connector = create_secure_hive_client(trusted_roots, Some((rogue_cert_chain, rogue_key))).unwrap();

    // 4. Client attempts to connect
    let cmd = HiveCommand::Pulse;
    let target_addr = format!("127.0.0.1:{}", target_port);
    
    let result = HiveClient::send_command_tls(
        &target_addr,
        cmd,
        Some(tls_connector),
        Some("localhost")
