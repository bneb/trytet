...
//! Requires pre-built SAT solver WASM (cargo component build in crates/sat-cartridge).
//!
...
/// Returns None if WASM not built (test skips gracefully).
fn load_sat_evaluator_wasm() -> Option<Vec<u8>> {
    let candidates = &[
        "crates/sat-cartridge/target/wasm32-wasip1/release/sat_cartridge.wasm",
        "dist/cartridges/sat_cartridge.wasm",
    ];
    ...
    eprintln!("SAT cartridge .wasm not found, skipping test");
    None
}
...
fn test_sat_evaluator_success() {
    ...
    let wasm = match load_sat_evaluator_wasm() {
        Some(w) => w,
        None => return,
    };

    mgr.precompile("sat-cartridge", &wasm)
        .expect("Failed to precompile sat-cartridge");
    ...
    assert!(
        output.contains(r#""satisfiable":true"#),
        "Output was: {}",
        output
    );
...
fn test_sat_evaluator_unsat() {
    ...
    assert!(
        output.contains(r#""satisfiable":false"#),
        "Output was: {}",
        output
    );
}
