...
async fn setup_mock_mcp_server() -> (tokio::io::DuplexStream, tokio::task::JoinHandle<()>) {
    ...
    let js_wasm = {
        let candidates = &[
            "crates/js-evaluator/target/wasm32-wasip1/release/js_evaluator.wasm",
            "dist/cartridges/js_evaluator.wasm",
        ];
        let mut wasm = None;
        for path_str in candidates {
            let path = std::env::current_dir().unwrap().join(path_str);
            if let Ok(w) = std::fs::read(&path) {
                wasm = Some(w);
                break;
            }
        }
        wasm.expect(
            "Missing js_evaluator.wasm — build with cargo component or copy to dist/cartridges/",
        )
    };

    sandbox
        .cartridge_manager
        .precompile("js-evaluator", &js_wasm)
        .unwrap();
...
async fn test_mcp_initialize() {
    let (client_stream, _handle) = setup_mock_mcp_server().await;
...
async fn test_mcp_tools_list() {
    ...
    let js_tool = tools
        .iter()
        .find(|t| t["name"] == "trytet_js_evaluator")
        .expect("JS Evaluator tool missing");
    assert_eq!(
        js_tool["description"].as_str().unwrap(),
...
async fn test_mcp_tools_call_execution() {
    ...
    assert!(
        !res_valid["result"]["isError"].as_bool().unwrap_or(false),
    ...
    assert!(res_malicious["result"]["isError"]
        .as_bool()
        .unwrap_or(false));
    let err_content = res_malicious["result"]["content"][0]["text"]
        .as_str()
        .unwrap();
    // Fuel exhaustion messages use human-readable English text.
    // The engine traps the infinite loop at the instruction level and returns
    // a structured error with isError: true.
    assert!(
        err_content.contains("fuel") || err_content.contains("Fuel"),
        "Engine must trap the infinite loop: {}",
        err_content
    );
}
