...
fn test_phase26_teleport_warp_ceiling() {
    ...
    if cfg!(debug_assertions) {
        eprintln!(
            "Teleport Warp (debug): {}ms (ceiling enforced in --release only)",
            warp_ms
        );
        return;
    }
    assert!(
        warp_ms < 2_000,
        "Teleport Warp EXCEEDED ceiling: {}ms (must be <2000ms)",
        warp_ms
    );
}
...
fn test_phase26_oracle_fidelity() {
    ...
    // Debug builds are too variable for hard ceilings; criterion benchmarks
    // in --release mode provide the authoritative measurement.
    if cfg!(debug_assertions) {
        eprintln!(
            "Oracle Fidelity (debug): {}µs (ceiling enforced in --release only)",
            verification_us
        );
        return;
    }
