...
async fn test_red_team_fuel_stealing() {
    ...
    let sandbox =
        Arc::new(WasmtimeSandbox::new(mesh.clone(), vm, false, "test_node".to_string()).unwrap());
    ...
    sandbox
        .cartridge_manager
        .precompile("quick-fail", &cartridge_wasm)
        .unwrap();
    ...
    assert!(
        res.fuel_consumed < 10000,
        "Red Team Alert: Cartridge execution errors steal all allocated fuel without refunding!"
    );
}
