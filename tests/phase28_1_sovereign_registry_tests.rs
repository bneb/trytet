...
use tet_core::registry::identity::{ArtifactManifest, IdentityRegistry, RegistryError};
use uuid::Uuid;
...
impl Default for MockGlobalRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
async fn test_signature_wall_rejection() {
    ...
    // We mock the local Identity Registry to track valid signatures.
    let registry_a =
        IdentityRegistry::new(Arc::clone(&dht) as Arc<dyn tet_core::gateway::GlobalRegistry>);
    ...
    let signature_payload = format!(
        "{}:{}:{}",
        manifest.alias, manifest.wasm_cid, manifest.timestamp
    );
    ...
    let _cid_a = registry_a
        .push_artifact(manifest.clone(), wasm_bytes)
        .await
        .expect("Original push should succeed");
    ...
    let bad_sig_payload = format!(
        "{}:{}:{}",
        bad_manifest.alias, bad_manifest.wasm_cid, bad_manifest.timestamp
    );
    bad_manifest.signature = signing_key_b
        .sign(bad_sig_payload.as_bytes())
        .to_bytes()
        .to_vec();

    let result = registry_a
        .push_artifact(bad_manifest, malicious_bytes)
        .await;
...
async fn test_p2p_locality_pull() {
    ...
    peers_b
        .add_peer(tet_core::hive::HiveNodeIdentity {
    ...
    let (pulled_manifest, pulled_bytes) = registry_b
        .pull_artifact("Beta", Some(peers_b))
        .await
        .expect("Node B should pull Beta successfully");
...
async fn test_gene_reconstruction() {
    ...
    let registry_b =
        IdentityRegistry::new(Arc::clone(&dht) as Arc<dyn tet_core::gateway::GlobalRegistry>);
    let (pulled_manifest, _) = registry_b
        .pull_artifact_from_mesh("Omega", &manifest)
        .await
