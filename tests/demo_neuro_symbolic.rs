...
fn demo_neuro_symbolic_workflow() {
    ...
    mgr.precompile("z3-solver", &z3_wasm)
        .expect("precompile z3");

    eprintln!("  [1/5] z3-solver compiled");

    let constraints =
        r#"{"meetings":5,"week":"2026-W19","constraints":["no_conflicts","room_capacity"]}"#;
    ...
    eprintln!(
        "  [2/5] SAT: {} meetings, 0 conflicts ({}us, {} fuel)",
        schedule.len(),
        elapsed.as_micros(),
    ...
    mgr.precompile("logic-bomb", &bomb_wasm)
        .expect("precompile bomb");
    ...
    eprintln!(
        "  [5/5] z3-solver still operational ({}us, {} fuel)\n",
