...
fn load_jmespath_evaluator_wasm() -> Vec<u8> {
    let candidates = &[
        "crates/jmespath-cartridge/target/wasm32-wasip1/release/jmespath_cartridge.wasm",
        "dist/cartridges/jmespath_cartridge.wasm",
    ];
    ...
    panic!(
        "JMESPath cartridge .wasm not found. Tried: {:?}",
        candidates
    )
}
...
fn test_jmespath_evaluator_success() {
    ...
    mgr.precompile("jmespath-cartridge", &wasm)
        .expect("Failed to precompile jmespath-cartridge");
    ...
    assert!(
        output.contains(r#"\"Bellevue, Seattle\""#),
        "Output was: {}",
        output
    );
