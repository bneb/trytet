...
async fn test_red_team_mcp_unbounded_read() {
    ...
    let (client_stream, server_stream) = tokio::io::duplex(1024 * 1024 * 50); // 50MB buffer
    ...
    let (_client_rx, mut client_tx) = tokio::io::split(client_stream);
    ...
    let _result = client_tx.write_all(&massive_payload).await;
...
async fn test_red_team_replay_path_traversal() {
    ...
    let _malicious_snapshot_id = "../secret";
